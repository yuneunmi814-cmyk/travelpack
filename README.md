...
> 🏆 **2026 관광데이터 활용 공모전(웹·앱 개발 부문) 준비 중** — 본선(10월) 목표. 2026-06-27 팀 회의에서 제품 정체성을 **여행팩(간편 여행 패키지)**로 확정하고, 학습 부담이 큰 바둑/스톤마크 게임 레이어는 제외했습니다(*관광지 자동 기록* 요소만 향후 가벼운 커뮤니티 기능으로 여지를 남김). 이어 06-28~06-30 미팅에서 **타겟을 데이터로 검증**(시장 1위 통증은 *코스 큐레이션이 아니라 예약·환불·결제*)하며 방향을 **외국인(태국 1순위·일본) 대상 "지역축제 여행팩"**으로 좁혔고, 서비스명을 **코타(KOTA = Korea Tiger) / Korea Festa**(마스코트 호랑이)로 확정했습니다. MVP는 **축제+교통+식당 3축의 웹앱**(검증 지역 고양·공주)으로, 구글맵의 한국 데이터 공백을 외국인 대상 경쟁력으로 삼습니다. 핵심 가치는 **공공데이터 신뢰 + 큐레이션으로 선택지 축소**. 문제정의(SCQA)·통합 전략·제안서 초안·실행계획·**회의록**은 [`docs/공모전기획/`](docs/공모전기획/) 참조.

> ℹ️ 아래 본문은 현재 구현된 **TravelPack 앱(코스 추천·가이드 모드)** 기준입니다. 공모전 방향은 위 회의를 거쳐 **코타(외국인·지역축제 웹앱)**로 진화 중이며, 두 흐름은 공공데이터 큐레이션 엔진을 공유합니다.
...
| 항목 | 내용 |
|---|---|
| **공모전** | 2026 관광데이터 활용 공모전 (웹·앱 개발 부문) — 본선 2026년 10월 |
| **주제** | 코타(Korea Festa) — 외국인(태국·일본)에게 한국 **지역축제 + 교통 + 식당**을 공공데이터로 큐레이션하는 여행팩 웹앱 |
| **데이터** | 한국관광공사 TourAPI 등 공공데이터 + 카카오/네이버 지도·플레이스 |

**팀 RNR**

> 개발 분담(백엔드 윤은미·프론트 최용우·UX/UI 전도준) 위에, 06-29 미팅에서 **사업 조직(마케팅·영업·시스템)**을 얹어 재편.

| 영역 | 담당 |
|---|---|
| 개발·배포 + 마케팅 (웹앱 구현·SEO·고객행동) | 윤은미 |
| 디버깅·QA + 영업 (지자체·식당 제휴) | 최용우 |
| PM·비즈니스 구조·시스템·디자인 (큐레이션·로고·레퍼런스) | 전도준 |

**협업 규칙** · 회의·자료공유=디스코드, 퀵 결정=카톡, 코드·작업물=GitHub(작업마다 README 로그). 의견엔 근거·대안 필수. 개발은 **윤은미 개발/배포 + 최용우 버그 리포트(한 번에 모아서) + 전도준 검수** 순환.

**회의 내용**

- **26.06.17.** — 주제 브레인스토밍: 여행/관광 도메인 아이디어 발산, 통합 기획 방향 탐색
- **26.06.27.** — 주제 선정: 제품을 **여행팩(간편 여행 패키지)**로 확정. 바둑/스톤마크 게임 레이어 제외, 지역 범위는 전국으로 시작, 타겟은 6/28 자료조사 후 확정. RNR·협업툴·데일리 미팅(매일 22:30)·10월 본선 역산 일정 합의. → [회의록 전문](docs/공모전기획/팀회의_회의록_2026-06-27.md)
- **26.06.28.** — 데일리 미팅 #1: 각자 자료조사 발표 후 타겟 전략 논쟁. 세 후보로 좁힘 — ① 아이동반 가족 + 공공데이터 신뢰 메타레이어·택시투어 송객(윤은미) ② 외국인(일본·동남아) 지역축제 바우처(전도준) ③ 반려동물/취약계층 무장애 여행(최용우). **외국인×지역축제(B안)에 무게**, 반려동물(C안)은 드래프트 1일·엔진 동일이라 병행 대안으로 보존. 액션 = 지역축제 시장조사 분담. → [회의록 전문](docs/공모전기획/팀회의_회의록_2026-06-28.md)
- **26.06.29.** — 데일리 미팅 #2: 지역축제 데이터 한계(외국인 집계 부재·개최일 불확실) 확인 → **검증 지역을 고양시·공주시로 확정**. RNR을 **마케팅(윤은미)·영업(최용우)·비즈니스 구조/시스템(전도준)**으로 재편. MVP는 **축제+교통+식당 3축의 웹**(숙소 제외, 에어비앤비 레퍼런스·실데이터)을 먼저 배포해 수요 검증. 초기 무료·수익구조 설계만 선행. → [회의록 전문](docs/공모전기획/팀회의_회의록_2026-06-29.md)
- **26.06.30.** — 데일리 미팅 #3: 축제 웹(윤은미)·식당 웹(최용우) 공유 → 큐레이션·편의시설 아이콘·SNS/태그·GPS 교통 연동 설계. 타겟을 **태국 1순위·일본 차순위**로 확정(채널 Pantip·인스타 등), 구글맵의 한국 데이터 공백을 경쟁력으로. **서비스명 "코타(KOTA = Korea Tiger) / Korea Festa"**(마스코트 호랑이) 결정. 협업은 윤은미 개발·배포 / 최용우 디버깅 / 전도준 PM·디자인. → [회의록 전문](docs/공모전기획/팀회의_회의록_2026-06-30.md)
- **26.07.01.** — 데일리 미팅 #4: 코타 웹에 식당·지도·검색·로그인/리뷰·추천 알고리즘·약관·모바일 반응형까지 붙음. **1인 개발/배포(윤은미)+디버깅 리포트(최용우)+검수·디자인(전도준)** 체계 확정. 디자인은 *구석구석을 반면교사로 단순·한국적*, **소셜보다 웹 최소 최적화 우선**. 로그인은 **카카오+구글+라인**, 번아웃 방지 **1일 1시간 리밋**. "현재 수준+버그만 잡아도 공모전 충족, 그 이상은 사업". → [회의록 전문](docs/공모전기획/팀회의_회의록_2026-07-01.md)
...
| [`docs/공모전기획/`](docs/공모전기획/) | 🏆 **2026 관광데이터 활용 공모전** 기획 패키지 — SCQA 문제정의·통합 브리프·제안서 초안·결정안건·실행계획 + **팀 회의록(6/27·6/28)** + 팀공유 PDF |
