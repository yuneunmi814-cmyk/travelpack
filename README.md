...
> 🏆 **2026 관광데이터 활용 공모전(웹·앱 개발 부문) 준비 중** — 본선(10월) 목표. 2026-06-27 팀 회의에서 제품 정체성을 **여행팩(간편 여행 패키지)**로 확정하고, 학습 부담이 큰 바둑/스톤마크 게임 레이어는 제외했습니다(*관광지 자동 기록* 요소만 향후 가벼운 커뮤니티 기능으로 여지를 남김). 이어 06-28 미팅에서 **타겟을 데이터로 검증**(앱리뷰 929건·검색량 실측)했고 — 시장의 1위 통증은 *코스 큐레이션이 아니라 예약·환불·결제*임을 확인 — **외국인(일본·동남아) 대상 지역축제 여행팩**을 잠정 1순위로, 반려동물 동반 여행을 병행 대안으로 두었습니다. 핵심 가치는 **공공데이터 신뢰 + 큐레이션으로 선택지 축소**. 문제정의(SCQA)·통합 전략·제안서 초안·실행계획·**회의록·타겟 리서치**는 [`docs/공모전기획/`](docs/공모전기획/) 참조.
...
| **주제** | 여행팩 — 핵심만 패키지로 큐레이션해 선택 피로를 줄이는 간편 여행 앱 |
...
| 백엔드 (앱 구현·DB·API) | 윤은미 |
| 프론트엔드 (화면 연결·동작) | 최용우 |
| UX/UI 디자인 (사용성·산출) | 전도준 |
| PM/팀장 | 보류 (추후 결정) |

**협업 규칙** · 회의·자료공유=디스코드, 퀵 결정=카톡, 코드·작업물=GitHub(작업마다 README 로그). 의견엔 근거·대안 필수.
...
- **26.06.28.** — 데일리 미팅 #1: 각자 자료조사 발표 후 타겟 전략 논쟁. 세 후보로 좁힘 — ① 아이동반 가족 + 공공데이터 신뢰 메타레이어·택시투어 송객(윤은미) ② 외국인(일본·동남아) 지역축제 바우처(전도준) ③ 반려동물/취약계층 무장애 여행(최용우). **외국인×지역축제(B안)에 무게**, 반려동물(C안)은 드래프트 1일·엔진 동일이라 병행 대안으로 보존. 액션 = 지역축제 시장조사(30선) 분담. → [회의록 전문](docs/공모전기획/팀회의_회의록_2026-06-28.md)
  - 회의 자료: [타겟·통증·솔루션 데이터 조사](docs/공모전기획/타겟조사_2026-06-28.md)(앱리뷰 929건 분석 — *진짜 1위 통증은 예약·환불·결제 ~50%, 큐레이션은 불만조차 아님*) · [타겟 전략 브리핑 카드](docs/공모전기획/타겟브리핑_카드_2026-06-28.html)(브라우저 열람) · [타겟지역 30선](docs/공모전기획/타겟지역_30선_2026-06-29.xlsx)(외국인 거점 서울+근교 지역축제 후보)
...
| [`docs/공모전기획/`](docs/공모전기획/) | 🏆 **2026 관광데이터 활용 공모전** 기획 패키지 — SCQA 문제정의·통합 브리프·제안서 초안·결정안건·실행계획 + **팀 회의록(6/27·6/28)·타겟 리서치 자료**(앱리뷰 929건 분석·타겟지역 30선) + 팀공유 PDF |
