...
1. **코스 찾기** — 홈의 추천 코스, 지역·테마 탐색, 또는 상단 🔍 통합 검색(코스·관광지·지역)으로 마음에 드는 코스를 찾습니다.
2. **코스 살펴보기** — 코스 상세에서 일자별 일정·지도·관광지 정보(운영시간·요금·꿀팁·사진·오디오 가이드, KO/EN)를 확인합니다.
...
- [x] **출시 준비** — PG(PortOne) 연동 코드·Docker 풀스택·CI·법무 초안·**통합 검색**·**영문 토글(KO/EN)**·**카카오맵 WebView 번호 마커/경로선** 완료. 사업자(프로젝트윤·간이과세자) 등록 완료 → **통신판매업 신고는 간이과세자 면제**, 위치기반서비스사업 신고가 출시 선행조건
