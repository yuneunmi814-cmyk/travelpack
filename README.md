...
> 🏆 **2026 관광데이터 활용 공모전(웹·앱 개발 부문) 준비 중** — 본선(10월) 목표. 2026-06-27 팀 회의에서 제품 정체성을 **여행팩(간편 여행 패키지)**로 확정하고, 학습 부담이 큰 바둑/스톤마크 게임 레이어는 제외했습니다(*관광지 자동 기록* 요소만 향후 가벼운 커뮤니티 기능으로 여지를 남김). 이어 06-28 미팅에서 **타겟을 데이터로 검증**했고 — 시장의 1위 통증은 *코스 큐레이션이 아니라 예약·환불·결제*임을 확인 — **외국인(일본·동남아) 대상 지역축제 여행팩**을 잠정 1순위로, 반려동물 동반 여행을 병행 대안으로 두었습니다. 핵심 가치는 **공공데이터 신뢰 + 큐레이션으로 선택지 축소**. 문제정의(SCQA)·통합 전략·제안서 초안·실행계획·**회의록**은 [`docs/공모전기획/`](docs/공모전기획/) 참조.
